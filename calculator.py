oop = open("/home/luka/My-codes/QtCalculator/qtcalculator/eqv.txt", "r")
lenred = open("/home/luka/My-codes/QtCalculator/qtcalculator/lineheight.txt", "r")
bnum = int(lenred.read())
...
def error_han(lin):
    ...
    while i < len(lin):
        if lin[i] != '+' and lin[i] != '-' and lin[i] != '*' and lin[i] != '/' and lin[i] != '0' and lin[i] != '1' and lin[i] != '2' and lin[i] != '3' and lin[i] != '4' and lin[i] != '5' and lin[i] != '6' and lin[i] != '7' and lin[i] != '8' and lin[i] != '9':
            error = "Given string contains invalid character/characters:"
            op = open("/home/luka/My-codes/QtCalculator/qtcalculator/ans.txt", "w")
            op.write(error)
            quit()
        elif (lin[i] == '+' or lin[i] == '-' or lin[i] == '*' or lin[i] == '/') and (lin[i + 1] == '+' or lin[i + 1] == '-' or lin[i + 1] == '*' or lin[i + 1] == '/'):
            error = "Given string have two or more operators that don't have number between them!"
            op = open("/home/luka/My-codes/QtCalculator/qtcalculator/ans.txt", "w")
            op.write(error)
            quit()
        elif lin[0] == '+' or lin[0] == '-' or lin[0] == '*' or lin[0] == '/':
            error = "Given string doesn't begins with number"
            op = open("/home/luka/My-codes/QtCalculator/qtcalculator/ans.txt", "w")
            op.write(error)
            quit()
        elif lin[len(lin) - 1] == '+' or lin[len(lin) - 1] == '-' or lin[len(lin) - 1] == '*' or lin[len(lin) - 1] == '/':
            error = "Given string doesn't ends with number"
            op = open("/home/luka/My-codes/QtCalculator/qtcalculator/ans.txt", "w")
            op.write(error)
            quit()
        elif lin[0] == '0' and i != len(lin) - 1 and (lin[1] != '+' and lin[1] != '-' and lin[1] != '*' and lin[1] != '/'):
            error = "Given string contains number/numbers that stars with '0'"
            op = open("/home/luka/My-codes/QtCalculator/qtcalculator/ans.txt", "w")
            op.write(error)
            quit()
        ...
        i = i + 1
    if n == 0:
        error = "Given string doesn't contains any operators"
        op = open("/home/luka/My-codes/QtCalculator/qtcalculator/ans.txt", "w")
        op.write(error)
        quit()
